...
public class StatsController {
    ...
    public ModelAndView admin(Model model, Authentication authentication) throws IOException {
        ...
        List<String> listTaskPartner = null;
        try {
           ...
           HttpEntity entity = response.getEntity();//получаем ответ от АПИ
            ...
            Set<String> setTsk = new LinkedHashSet<>();//получение контрагентов
            for(Tasks tsk: task.getTasks())
            {
                setTsk.add(tsk.getTaskPartner());
            }
            listTaskPartner = new LinkedList<>(setTsk);
        ...
        resultStats = new ResultStats();
        modelAndView.addObject("resultTask",resultStats.taskResult(task));
        modelAndView.addObject("allTaskPartner",listTaskPartner);
